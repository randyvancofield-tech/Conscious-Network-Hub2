...
const fetchRemoteHCNKnowledge = async (): Promise<HCNKnowledge | null> => {
  try {
    const response = await fetch(HCN_KNOWLEDGE_URL, {
      ...
      // Use no-cors mode only if needed for CORS issues
      mode: 'cors',
    });
...
/**
 * Get local fallback HCN knowledge (imported statically)
 */
const getLocalHCNKnowledge = (): HCNKnowledge | null => {
  // This is imported from the bundled JSON at build time
  try {
    return require('../knowledge/hcn_scope.json');
  } catch (error) {
    console.error('HCN Knowledge: Local knowledge unavailable', error);
    return null;
  }
};
