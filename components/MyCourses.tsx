...
const MyCourses: React.FC<MyCoursesProps> = ({ enrolledCourses, onNavigateToUniversity, onGoDashboard, onUpdateProgress }) => {
  ...
  return (
    <div
      className="mx-auto w-full min-w-0 space-y-8 overflow-x-hidden animate-in fade-in duration-500"
      style={{ maxWidth: 'calc(100vw - 2rem)' }}
    >
      <header className="flex min-w-0 flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div className="min-w-0">
          <h2 className="mb-2 break-words text-[clamp(1.75rem,7vw,2rem)] font-bold leading-tight text-white sm:text-3xl">My Sovereign Learning</h2>
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-3">
          <button
            ...
            className="inline-flex min-w-0 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-200 transition-all hover:bg-white/10"
          >
            <Home className="h-4 w-4" />
            <span className="cnh-action-label">Dashboard</span>
          </button>
          <div className="flex min-w-0 items-center gap-2 rounded-xl border border-blue-500/20 bg-blue-600/10 px-4 py-2">
